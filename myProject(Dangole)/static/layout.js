...
$(document).ready(function() {
        ...
        if(response['result'] == 'success'){
          ...
          for(let i = 0; i<titles.length; i++){
            ...
            let temp_html = `<label for="restaurant" class="col-form-label">영상 이름 : </label>
              ...
              <input type="text" class="form-control" name="thumbnail" value="${thumbnail}" disabled>

              <label for="restaurant" class="col-form-label">영상 Id : </label>
              <input type="text" class="form-control" name="videoId" value="${videoId}" disabled>`
  ...
  function getInputValue(){
    ...
    $.ajax({
      type: "POST",
      url: "/list/saveMemo",
      data: {'title_give':title, 'thumbnail_give':thumbnail, 'videoId_give':videoId, 
